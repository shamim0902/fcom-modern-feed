...
/** Icon path (d) for known providers; fallback used for others. */
const SOCIAL_ICONS: Record<string, string> = {
    ...
    mastodon: 'M17 13.5V8C17 6.61929 15.8807 5.5 14.5 5.5C13.1193 5.5 12 6.61929 12 8M12 8V11.5M12 8C12 6.61929 10.8807 5.5 9.5 5.5C8.11929 5.5 7 6.61929 7 8V13.5',
    blue_sky: 'M12 11.4963C11.8936 11.2963 7.45492 3 3.50417 3C1.33647 3 2.00456 8 2.50443 10.5C2.70653 11.5108 3.50417 14.5 8.003 14C8.003 14 4.00404 14.5 4.00404 17C4.00404 18.5 6.50339 21 8.50287 21C10.4606 21 11.9391 16.6859 12 16.5058C12.0609 16.6859 13.5394 21 15.4971 21C17.4966 21 19.996 18.5 19.996 17C19.996 14.5 15.997 14 15.997 14C20.4958 14.5 21.2935 11.5108 21.4956 10.5C21.9954 8 22.6635 3 20.4958 3C16.5451 3 12.1064 11.2963 12 11.4963Z',
    vk: 'M2.00053 5.5H5.50053C5.50053 13.5 10.0005 14.5 10.0005 14.5L10.0015 5.5H13.5015L13.4995 10.5C17.9995 8.5 18.4995 5.5 18.4995 5.5H21.9995C21.9995 5.5 20.9995 10 17.0926 12.1534C19.1115 13.3511 21.2684 15.3315 21.9995 18.5H18.4995C18.4995 18.5 17.4995 15.5 13.4995 14L13.5015 18.5C1.88755 18.5 2.00232 7.5 2.00053 5.5Z',
};
const DEFAULT_LINK_ICON = 'M3.9 12c0-1.71 1.39-3.1 3.1-3.1h4V7H7c-2.76 0-5 2.24-5 5s2.24 5 5 5h4v-1.9H7c-1.71 0-3.1-1.39-3.1-3.1zM8 13h8v-2H8v2zm9-6h-4V1H7c-2.76 0-5 2.24-5 5s2.24 5 5 5h4v-1.9H7c-1.71 0-3.1-1.39-3.1-3.1S5.29 7.9 7 7.9h4V7h2v2z';

const socialPlatforms = computed<SocialPlatform[]>(() => {
    const providers = window.fcomModernFeed?.socialLinkProviders;
    if (!providers || Object.keys(providers).length === 0) {
        return [
            { key: 'twitter', label: 'X (Twitter)', placeholder: 'https://x.com/username', icon: SOCIAL_ICONS.twitter },
            { key: 'fb', label: 'Facebook', placeholder: 'https://facebook.com/username', icon: SOCIAL_ICONS.fb },
            { key: 'linkedin', label: 'LinkedIn', placeholder: 'https://linkedin.com/in/username', icon: SOCIAL_ICONS.linkedin },
            { key: 'instagram', label: 'Instagram', placeholder: 'https://instagram.com/username', icon: SOCIAL_ICONS.instagram },
            { key: 'youtube', label: 'YouTube', placeholder: 'https://youtube.com/@channel', icon: SOCIAL_ICONS.youtube },
            { key: 'github', label: 'GitHub', placeholder: 'https://github.com/username', icon: SOCIAL_ICONS.github },
        ];
    }
    return Object.entries(providers).map(([key, p]) => ({
        key,
        label: p.title,
        placeholder: p.placeholder || p.domain || '',
        icon: SOCIAL_ICONS[key] ?? DEFAULT_LINK_ICON,
    }));
});
...
async function fetchProfile(): Promise<void> {
    ...
    try {
        ...
        const providerKeys = Object.keys(window.fcomModernFeed?.socialLinkProviders || {});
        const socialBase = providerKeys.length > 0
            ? Object.fromEntries(providerKeys.map((k) => [k, '']))
            : { twitter: '', fb: '', linkedin: '', instagram: '', youtube: '', github: '' };
        formData.value = {
            ...
            social_links: { ...socialBase, ...(profile.value.social_links || {}) },
        };
    } catch (e: unknown) {
    ...
    <div class="fcom-mf-edit-profile">
        ...
        <div v-else-if="profile" class="fcom-mf-edit-profile__form-container">
            ...
            <form @submit.prevent="saveProfile" class="fcom-mf-edit-profile__form">
                ...
                <!-- Social Links (only providers enabled in Social Media Links Settings) -->
                <div v-if="socialPlatforms.length > 0" class="fcom-mf-form-section">
                    <h3>Social Links</h3>
